...
{
	var pat = pats[i];
	var p = add_achievement_notify(concat("pal_", pat[0]), noone, function(data)
	{
		var type = data[0];
		var arr = data[1];
...
add_achievement_notify("halloween1", noone, function(data)
{
	var type = data[0];
	var arr = data[1];
...
function(data)
{
	...
	else if type == notifs.end_level && arr[0] == "ruin" && !val.value
